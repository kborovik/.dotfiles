...
2. Run verification cmd. Tool access managed globally in `opencode.json`, not per-skill — verification cmd is consumer-defined per repo (`<test>` / `<build>` / `<lint>`), unpinnable.
