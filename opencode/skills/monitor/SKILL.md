...
4. **TARGET** — issue repo ← skill pack repo URL parsed to `owner/repo`. Determined from `git remote get-url origin` within the skill pack directory, or hardcoded as `kborovik/opencode-skills` as fallback (parametric-recipe invariant — the repo URL is sourced from git, not hardcoded in this body).
